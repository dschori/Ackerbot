...
from .positions import pos_env_train, pos_env_val, pos_custom
...
class NavigationEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def reset(self):
        ...
        env_type = 'train'
        if env_type == 'train':
            self.initial_position, self.target_p = pos_env_train()
        elif env_type == 'val':
            self.initial_position, self.target_p = pos_env_val('forward')
        elif env_type == 'custom':
            self.initial_position, self.target_p = pos_custom(starting_pos=self.starting_pos,
                                                              start_orientation=self.start_orientation,
                                                              target_pos=self.target_pos)
        else:
            raise ValueError('Bad Env_type')
